...
1. (claim-finite-table-misses-inputs) For any finite lookup table there exist finite inputs outside
   its stored domain; to such an input it can at most respond by ignoring the part of the input that
   exceeds its keys.
2. (claim-mind-handles-unbounded-inputs) A genuine mind responds appropriately to such inputs rather
   than discarding their excess.
∴ (claim-lookup-table-combinatorially-inadequate) For every finite table there is an input on which
   it diverges from the mind; so no finite table reproduces a mind's full behavioural profile.

bearing on the target: this `responds_to` and `attacks` argument-lookup-table-objection by denying
